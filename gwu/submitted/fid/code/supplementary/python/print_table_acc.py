...
def print_table_acc():
    ...
    datasets = {'iris': 'Iris',
                ...
                'drosophila_enhancers': 'Drosophila enhancers',
                'alternative_splicing': 'Alternative splicing'}

    methods = {'LogisticRegression': 'LR',
               'GaussianNB': 'GNB',
               'fiblr': 'FIBLR'}
    ...
    files_others = [file for file in files if
                    '/cv_results_file/' in os.path.abspath(file)]

    # Get the score file of fiblr
    files_fiblr = [file for file in files if
                   '/fiblr/' in os.path.abspath(file) and '/cv_results_file/' in os.path.abspath(file)]
    ...
    with open(table_file, 'w') as f:
        # Table content
        # Write the methods
        content = 'Dataset' + '\t' + '&' + '\t&'.join([methods[method] for method in methods.keys()]) + '\\\\'
        f.write(content + '\n')
        ...
        for dataset in datasets.keys():
            ...
            for method in methods.keys():
                ...
                dataset_method_score_files_fiblr = get_score_files(files_fiblr, dataset, '')
                ...
                if len(dataset_method_score_files_fiblr) > 0:
                    nobs_fiblr, mean_fiblr, std_fiblr = get_nobs_mean_std(dataset_method_score_files_fiblr)
                else:
                    nobs_fiblr, mean_fiblr, std_fiblr = None, None, None

                if mean_others is not None and mean_fiblr is not None:
                    dif = mean_others - mean_fiblr

                    if dif < 0:
                        score = str(format(mean_others * 100, '.' + str(num_decimal_places) + 'f')) + '\\%'
                    elif dif > 0:
                        score = str(format(mean_others * 100, '.' + str(num_decimal_places) + 'f')) + '\\%'
                    else:
                        score = str(format(mean_others * 100, '.' + str(num_decimal_places) + 'f')) + '\\%'
                    ...
                    if (std_others is not None
                            and std_fiblr is not None
                            and std_others + std_fiblr != 0):
                        # significance test

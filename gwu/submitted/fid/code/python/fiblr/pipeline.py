...
def pipeline_one_dataset(dp, data_files, names_file):
    ...
    pipe_fiblr = Pipeline([('scaler', setting.scaler),
                            ('fiblr', FIBLR.FIBLR(setting.max_iter,
    ...
    gs = GridSearchCV(estimator=pipe_fiblr,
                      param_grid=[{'fiblr__bin_num_percent': setting.bin_num_percents,
                                   'fiblr__eta': setting.etas}],
...
def get_results(setting, names, data, gs):
    ...
    if setting.prob_dists_fig_dir is not None:
        ...
        plot_prob_dists_fig(setting, names, data.X, gs.best_estimator_.named_steps['fiblr'])

    if setting.prob_dists_file_dir is not None:
        # Write the probability distribution file
        write_prob_dists_file(setting, names, data.X, gs.best_estimator_.named_steps['fiblr'])
...
def plot_prob_dists_fig(setting, names, X, fiblr):
    """
    Plot the probability distribution figures.
    :param setting: the Setting object
    :param names: the Names object
    :param X: the feature matrix
    :param fiblr: the fiblr model
    :return:
    """
    ...
    for class_ in sorted(fiblr.weights.keys()):
        ...
        for j in sorted(fiblr.prob_dists[class_].keys()):
            ...
            pijs = [fiblr.prob_dists[class_][j][i] for xij, i in xij_i_pairs_sorted]
...
def write_prob_dists_file(setting, names, X, fiblr):
    """
    Write the probability distribution file
    :param setting: the Setting object
    :param names: the Names object

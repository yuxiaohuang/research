...
name_L = ['age of the patient', 'spectacle prescription', 'astigmatic', 'tear production rate', 'class']

val_LL = ['1, 2, 3', '1, 2', '1, 2', '1, 2', '1, 2']
...
def generate_data():
    ...
    for i in range(0, max_time_stamp):
        ...
        for j in range(len(spamreader[i])):
            if spamreader[i][j] != '':
                line_L.append(spamreader[i][j])
        # Check length
        if len(line_L) != len(name_L) + 1:

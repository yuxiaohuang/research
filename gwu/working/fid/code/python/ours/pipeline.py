...
def get_results(setting, names, data, gs):
    ...
    if setting.prob_dists_fig_dir is not None:
        ...
        plot_prob_dists_fig(setting, names, data.X, gs.best_estimator_.named_steps['fid'])

    if setting.prob_dists_file_dir is not None:
        # Write the probability distribution file
        write_prob_dists_file(setting, names, data.X, gs.best_estimator_.named_steps['fid'])
...
def plot_prob_dists_fig(setting, names, X, fid):
    ...
    for class_ in sorted(fid.weights.keys()):
        ...
        for j in sorted(fid.prob_dists[class_].keys()):
            ...
            pijs = [fid.prob_dists[class_][j][xij] for xij in np.unique(sorted(fid.prob_dists[class_][j].keys()))]
...
def write_prob_dists_file(setting, names, X, fid):
    """
    Write the probability distribution file
    :param setting: the Setting object
    :param names: the Names object
    ...
    with open(prob_dists_file, 'w') as f:
        ...
        for class_ in sorted(fid.weights.keys()):
            ...
            for j in sorted(fid.prob_dists[class_].keys()):
                ...
                # Get the probabilities
                pijs = [fid.prob_dists[class_][j][xij] for xij in
                        np.unique(sorted(fid.prob_dists[class_][j].keys()))]

...
def pipeline_one_dataset(dp, data_files, names_file):
    ...
    # Set backend="multiprocessing" (default) to prevent sharing memory between parent and threads
    Parallel(n_jobs=1)(delayed(pipeline_one_classifier)(setting, data, clf_name)
                       for clf_name in setting.classifiers.keys())


def pipeline_one_classifier(setting, data, clf_name):
    """
    ...
    # gs_clf.fit(data.X, data.y)

    # Get the sklearn pipeline for RandomPARC
    # pipe_RandomPARC = Pipeline([(setting.name, RandomPARC.RandomPARC(base=gs_clf.best_estimator_))])
    pipe_RandomPARC = Pipeline([(setting.name, RandomPARC.RandomPARC())])

    pipe_RandomPARC.fit(data.X, data.y)
    pipe_RandomPARC.predict(data.X)

    # Hyperparameter tuning using GridSearchCV for RandomPARC
    ...
    # Get the results
    get_results(setting, clf_name, gs_clf, gs_RandomPARC)


def get_results(setting, clf_name, gs_clf, gs_RandomPARC):
    """
    Get the results
    :param setting: the Setting object
    :param clf_name: the name of the classifier
    ...
    if setting.sig_rule_file_dir is not None:
        ...
        write_sig_rule_file(setting, setting.name, gs_RandomPARC)
...
def write_sig_rule_file(setting, name, gs):
    """
    Write the significant rule file
    :param setting: the Setting object
    :param name: the name of the model
    ...
    sig_rules = pd.DataFrame.from_dict(gs.named_steps[setting.name].sig_rules)
    sig_rules.to_csv(path_or_buf=sig_rule_file)

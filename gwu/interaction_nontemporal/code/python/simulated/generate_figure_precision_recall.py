...
# The ground truth interactions for count 3 and 4
interaction_ground_truth_count_3_LL = [['src_10_1', 0, 0], ['src_52_1', 0, 0], ['src_11_1', 0, 0], ['src_29_0', 0, 0], ['src_63_0', 0, 0]]
    ...
    for y in sorted(y_Dic.keys()):
        ...
        while True:
            ...
            if sample_size_cutoff_met_F is False and sufficient_F is True:
                ...
                global precision_L, recall_L
                print(['precision_L: ', precision_L])
                print(['recall_L: ', recall_L])
                spamwriter_interaction.writerow(['precision_L: ', precision_L])
                spamwriter_interaction.writerow(['recall_L: ', recall_L])
                f_interaction.flush()
...
# Update the list of precision and recall
def get_precision_recall(X_L):
    if count == 3:
        interaction_ground_truth_LL = interaction_ground_truth_count_3_LL
    elif count == 4:
        interaction_ground_truth_LL = interaction_ground_truth_count_4_LL

    # Calculate true positive
    tp = 0
    for index in X_L:
        if x_LL[index] in interaction_ground_truth_LL:
            tp += 1

    # Calculate precision
    precision = float(tp) / float(len(X_L))
    # Update the list of precision
    global precision_L
    precision_L.append(precision)

    # Calculate recall
    recall = float(tp) / float(len(interaction_ground_truth_LL))
    # Update the lsit of recall
    global recall_L
